...
void buffer_set_arena_size(int width_in, int height_in) {
  arena_width = width_in + BORDER_WIDTH;
  arena_height = height_in + BORDER_WIDTH;
...
int buffer_index_from_x_y(int x, int y) {
  int i_x, i_y;
  i_x = (x + BORDER_WIDTH)/BUFFER_FACTOR;
  i_y = (y + BORDER_WIDTH)/BUFFER_FACTOR;
...
int x_from_buffer_index(int index) {
  return (index % buffer_width - BORDER_WIDTH)*BUFFER_FACTOR;
}
